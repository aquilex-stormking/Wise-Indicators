...
import { CoulumbiaService } from '../../../services/columbia/coulumbia.service';
...
export default class ViewTransitionComponent implements OnInit{
  ...
  constructor(private columbiaService:CoulumbiaService ) {  }
  ngOnInit(): void {
   this.get();
  }
  ...
  public get(){
    this.isLoading = true; 
    this.columbiaService.getData().subscribe(
      {
      next:(colum:Coulumbia|undefined) =>{
        this.coulumbia = colum;
        this.resultados = this.coulumbia?.properties.periods
